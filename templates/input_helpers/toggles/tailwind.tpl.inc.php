...
$required_colors = array('bg', 'focus:ring');
...
echo <<<HTML
<div {$show_fmt} x-data="{ enabled: {$checked_fmt} }" class="flex items-center justify-between {$width_fmt}">
...
echo <<<HTML
  ...
  <input x-model="enabled"
         id="{$parameters['id']}"
         name="{$parameters['name']}"
         type="hidden" />
  <button @click="enabled = !enabled"
          type="button"
          :class="{'{$colors['bg']}': enabled, 'bg-gray-200': !enabled}"
